...
class Pendaftaran extends CI_Controller
{
    ...
    //     $error = $this->db->error();
    //     if ($error['code'] != 0) {
    //         $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert"><i class="icon fas fa-info-circle"></i>Data Pendaftaran Tidak Dapat Dihapus! (Sudah Berelasi)</div>');
    //     } else {
    //         $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-check-circle"></i>Data Pendaftaran Berhasil Dihapus!</div>');
    //     }
    //     redirect('Pendaftaran');
    // }
    ...
    // {
    //     $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
    //     $data['daftar'] = $this->Pendaftaran_model->getById($id);
    //     $this->form_validation->set_rules('nama_siswa', 'Nama Siswa', 'required', ['required' => 'Nama Siswa Wajib di Isi']);
    //     $this->form_validation->set_rules('asal_sekolah', 'Asal Sekolah', 'required', ['required' => 'Asal Sekolah Wajib di Isi']);
    //     $this->form_validation->set_rules('tempat_lahir', 'Tempat Lahir', 'required', ['required' => 'Tempat Lahir Wajib di Isi']);
    //     $this->form_validation->set_rules('tgl_lahir', 'Tanggal Lahir', 'required', ['required' => 'Tanggal Lahir Wajib di Isi']);
    //     $this->form_validation->set_rules('agama', 'Agama', 'required', ['required' => 'Agama Wajib di Isi']);
    ...
    //         $this->Pendaftaran_model->update(['id_pendaftar' => $id_pendaftar], $data);
    //         $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Pendaftaran Berhasil DI Ubah!</div>');
    //         redirect('Pendaftaran');
    //     }
    // }
}
